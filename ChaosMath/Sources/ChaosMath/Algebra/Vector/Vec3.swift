//
//  CMvec4.swift
//  Enlighted
//
//  Created by Fu Lam Diep on 07.04.21.
//

import Foundation

public struct t_vec3<Component: CodableSignedNumeric>: StaticVector {

    public static var dimension: Int { 3 }

    public private(set) var components: [Component]

    public var x: Component {
        get { components[0] }
        set { components[0] = newValue }
    }

    public var y: Component {
        get { components[1] }
        set { components[1] = newValue }
    }

    public var z: Component {
        get { components[2] }
        set { components[2] = newValue }
    }

    public init (components: [Component]) {
        self.init()
        for index in 0..<min(dimension, components.count) {
            self.components[index] = components[index]
        }
    }

    public init<P>(from p1: P, to p2: P) where P : Point, Component == P.Component {
        self.init(components: p2.components)
        for index in 0..<min(dimension, p1.dimension) {
            self.components[index] -= p1.components[index]
        }
    }

    public init (_ x: Component = 0, _ y: Component = 0, _ z: Component = 0) {
        components = [x, y, z]
    }

    public init (from p1: t_point3<Component>, to p2: t_point3<Component>) {
        self.init(
            p2.components[0] - p1.components[0],
            p2.components[1] - p1.components[1],
            p2.components[2] - p1.components[2]
        )
    }

    public subscript(_ index: Int) -> Component {
        get { components[index] }
        set { components[index] = newValue }
    }

    public func dot(_ other: t_vec3<Component>) -> Component {
        let a = components[0] * other.components[0]
        let b = components[1] * other.components[1]
        let c = components[2] * other.components[2]
        return a + b + c
    }

    public func cross (_ other: Self) -> Self {
        Self(components: [
            components[1] * other.components[2] - components[2] * other.components[1],
            components[2] * other.components[0] - components[0] * other.components[2],
            components[0] * other.components[1] - components[1] * other.components[0]
        ])
    }

    @inlinable public static func + (_ lhs: Self, _ rhs: Self) -> Self {
        Self(
            lhs.components[0] + rhs.components[0],
            lhs.components[1] + rhs.components[1],
            lhs.components[2] + rhs.components[2]
        )
    }

    @inlinable public static prefix func - (_ vector: Self) -> Self {
        Self(
            -vector.components[0],
            -vector.components[1],
            -vector.components[2]
        )
    }

    @inlinable public static func * (_ vector: Self, _ scalar: Component) -> Self {
        Self(
            vector.components[0] * scalar,
            vector.components[1] * scalar,
            vector.components[2] * scalar
        )
    }
}

public extension t_vec3 {
    static var zero: Self { t_vec3() }
}

public extension t_vec3 where Component: FloatingPoint {
    static var infinity: Self { t_vec3(.infinity, .infinity, .infinity) }
}

extension t_vec3: FloatingPointVector where Component: FloatingPoint {

}

extension t_vec3: StaticFloatingPointVector where Component: FloatingPoint {

}

public typealias Vec3f = t_vec3<Float>
public typealias Vec3d = t_vec3<Double>
public typealias Vec3 = Vec3f
